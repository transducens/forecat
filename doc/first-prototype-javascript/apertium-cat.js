...
function translate(sourceSegments,startPos,maxPos) {
	return $.ajax({
		type: "GET",
		url: "http://api.apertium.org/json/translate",
		data: {q:sourceSegments, langpair:sourceLang+"|"+targetLang,key: apertiumApiKey,markUnknown: "no"},
		dataType: "jsonp",
		...
		success: function(data) {
			...
			// Last block of translations is responsible of removing the trobber:
			if (startPos==maxPos-1) {
...
function sliceAndTranslate() {
	...
	if (wordsCount*maxLength>maxSegments) {
		...
		wordsCount= Math.floor(maxSegments/maxLength);
	} 

	$("#wait2").html("<img src='Throbber-small.gif'/> Gathering translations. You may start typing now, though...").fadeIn();
	...
	$.each(words,function(i,word) {
		if (i<wordsCount) {
			...
			for (var j=1;j<=maxLength;j++) {
				...
				// TO-DO: group more segments until the limit of 2KB supported by Safari
			}
			...
			// To avoid sequential calls, just do this (also remove the "return" in the "return $ajax" in
			// the translate() function):
			// translate(sourceSegments,i);

			// To make sequential calls, we use JQuery deferred:
			// See http://stackoverflow.com/questions/8612894/how-to-chain-ajax-calls-using-jquery
			// Also, be careful with closures like this and iterating loops: 
			// http://www.mennovanslooten.nl/blog/post/62
			startingpoint=startingpoint.pipe( function() {
				console.log("Making request for segments starting at '" + words[i] + "' (position "+i+")");
				return translate(sourceSegments,i,wordsCount);
    		});
    	}
	});
}
